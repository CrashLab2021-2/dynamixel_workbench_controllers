...
void DynamixelController::initSubscriber()
{
  ...
  door2 = node_handle_.subscribe("door2", 1, &DynamixelController::doorOpen, this);
  door1 = node_handle_.subscribe("door1", 1, &DynamixelController::singleOpen, this);
}
...
void DynamixelController::initServer()
{
  ...
  door = node_handle_.advertiseService("door", &DynamixelController::doorCallback, this);
  tilt = node_handle_.advertiseService("tilt", &DynamixelController::tiltCallback, this);
  dispenser = node_handle_.advertiseService("dispenser", &DynamixelController::dispenserCallback, this);
}
...
bool DynamixelController::doorCallback(kanu_msgs::door::Request &req, kanu_msgs::door::Response &res){
  ...
  uint8_t id_array[2] = {2, 3};
  std::string item_name = "Moving_Speed";
  int32_t moving_speed = 150;
  int32_t angle = 350;

  for(int i=0; i<2; i++){ // 속도 설정
    result = dxl_wb_->itemWrite(id_array[i], item_name.c_str(), moving_speed, &log);
    if (result == false){
      ROS_ERROR("%s", log);
      ROS_ERROR("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", moving_speed, item_name.c_str(), id_array[i]);
    }
  }
...
bool DynamixelController::tiltCallback(kanu_msgs::tilt::Request &req, kanu_msgs::tilt::Response &res){
  ...
  uint8_t id = 4;
  std::string item_name = "Moving_Speed";
  int32_t value = 150; // Moving_Speed
  result = dxl_wb_->itemWrite(id, item_name.c_str(), value, &log);
    if (result == false){
      ROS_ERROR("%s", log);
      ROS_ERROR("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", value, item_name.c_str(), id);
...
bool DynamixelController::dispenserCallback(kanu_msgs::dispenser::Request &req, kanu_msgs::dispenser::Response &res){
  ...
  ros::Duration(1.7).sleep(); // 2초 후 정지
...
void DynamixelController::singleOpen(const std_msgs::Int16::ConstPtr &msg){
  bool result = false;
  const char* log;
...
void DynamixelController::doorOpen(const std_msgs::Int16::ConstPtr &msg){
  bool result = false;
  const char* log = NULL;
  ...
  int32_t value = 150;
  for(int i=0; i<2; i++){
    result = dxl_wb_->itemWrite(id_array[i], item_name.c_str(), value, &log);
    if (result == false)
    {
      ROS_ERROR("%s", log);
      ROS_ERROR("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", value, item_name.c_str(), id_array[i]);
    }
  }

  int32_t dynamixel_position[2] ={512 + msg->data, 512 - msg->data};
  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array, 2, dynamixel_position, 1, &log);
  if (result == false)  ROS_ERROR("%s", log);
}
